  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (items.length > 0) {
        ...
        alert("Success! But this album seems empty.");
        setPhotos([]);
        setStep('gallery');
      }
    } catch (e: any) {
      console.error(e);
      if (e.message.includes("INCORRECT_PIN")) {
          alert("Incorrect PIN. Please try again.");
      } else {
          alert("Connection Error. Please check link.");
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="p-2 grid grid-cols-3 md:grid-cols-5 lg:grid-cols-8 gap-1">
        {photos.length === 0 && <div className="col-span-full text-center p-10 text-gray-500">No photos to show.</div>}
        {photos.map((photo, i) => {
